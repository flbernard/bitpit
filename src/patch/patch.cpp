...
Patch::Patch(const int &id, const int &dimension)
	: m_nVertices(0), m_nInternals(0), m_nGhosts(0), m_nInterfaces(0),
	  ...
	  m_dirty(true), m_advancedEditable(false), m_hasCustomTolerance(false)
{
	set_id(id) ;
...
/*!
	Enables or disables advanced editing.

	When advance editing is enabled, it will be possible to change the
	patch using low level functions (e.g., it will be possible to
	add individual cells, add vertices, delete cells, ...).

	\param editable if true, the advanced editing will be enabled
*/
void Patch::setAdvancedEditable(bool editable)
{
	if (isAdvancedEditable() == editable) {
		return;
	}

	m_advancedEditable = editable;
}

/*!
	Checks if the advance editing is enabled.

	When advance editing is enabled, it will be possible to change the
	patch using low level functions (e.g., it will be possible to
	add individual cells, add vertices, delete cells, ...).

	\return This method returns true when the advanced editing is enabled,
	otherwise it returns false.
*/
bool Patch::isAdvancedEditable() const
{
	return m_advancedEditable;
}
